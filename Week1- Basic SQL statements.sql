...
CREATE TABLE jurisdiction_hierarchy (
    ...
    jurisdiction_location VARCHAR(30) NOT NULL,
    hierarchy VARCHAR(30),
    jurisdiction_name VARCHAR(30)
);


INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(1,'Middle Shabelle','Region',NULL);
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(2,'Hiraan','Region',NULL);
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(3,'Balcad','District','Middle Shabelle');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(4,'Jowhar','District','Middle Shabelle');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(5,'Beledweyn','District','Hiraan');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(6,'Dharkenley','Village','Beledweyn');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(7,'Bulo-Kahin','Village','Beledweyn');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(8,'Hilo Kelyo','Village','Beledweyn');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(9,'Xubow','Village','Beledweyn');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(10,'Xiintooy','Village','Beledweyn');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(11,'Dhagax Jebis','Village','Beledweyn');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(12,'Filtare','Village','Beledweyn');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(13,'Howl-Wadaag','Village','Beledweyn');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(14,'Balcad','Village','Balcad');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(15,'Mareeray','Village','Balcad');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(16,'Kulmis','Village','Balcad');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(17,'Sabuun','Village','Jowhar');
INSERT INTO jurisdiction_hierarchy (hierarchy_id, jurisdiction_location, hierarchy, jurisdiction_name) VALUES(18,'Bayaxaw','Village','Jowhar');
# Write a SQL script that creates the following summary tables (can be tables or views)

# District summary
CREATE VIEW District_summary AS
WITH DistrictBeneficiaries AS (
    SELECT 
        jh.jurisdiction_location AS district_name,
        jh.jurisdiction_name AS region_name,
        SUM(CASE 
    ...
    JOIN 
        jurisdiction_hierarchy jh ON bpd.village = jh.jurisdiction_location
    WHERE 
        jh.hierarchy = 'District'
    GROUP BY 
        jh.jurisdiction_location, jh.jurisdiction_name
),
DistrictPopulation AS (
    SELECT 
        jh.jurisdiction_location AS district_name,
    ...
    JOIN 
        jurisdiction_hierarchy jh ON vl.village = jh.jurisdiction_location
    WHERE 
        jh.hierarchy = 'Village'
    GROUP BY 
        jh.jurisdiction_location
)
SELECT
    db.district_name,
    db.region_name,
    db.total_individual_beneficiaries,
    COALESCE(dp.total_district_population, 0) AS total_district_population,
    CASE 
        WHEN dp.total_district_population > 0 THEN
            db.total_individual_beneficiaries / dp.total_district_population
        ELSE 
            0
    END AS individual_beneficiaries_ratio
FROM 
    DistrictBeneficiaries db
LEFT JOIN 
    DistrictPopulation dp ON db.district_name = dp.district_name;
    
# Partner summary    
CREATE VIEW Partner_summary AS
WITH PartnerVillages AS (
    SELECT 
        bpd.partner AS partner_name,
        COUNT(DISTINCT jh.jurisdiction_location) AS num_villages_reached
    FROM 
        beneficiary_partner_data bpd
    JOIN 
        jurisdiction_hierarchy jh ON bpd.village = jh.jurisdiction_location
...
PartnerDistricts AS (
    SELECT 
        bpd.partner AS partner_name,
        COUNT(DISTINCT jh.jurisdiction_name) AS num_districts_reached
    FROM 
        beneficiary_partner_data bpd
    JOIN 
        jurisdiction_hierarchy jh ON bpd.village = jh.jurisdiction_location
...
SELECT
    pv.partner_name,
    COALESCE(pv.num_villages_reached, 0) AS num_villages_reached,
    COALESCE(pd.num_districts_reached, 0) AS num_districts_reached
FROM 
    PartnerVillages pv
LEFT JOIN 
    PartnerDistricts pd ON pv.partner_name = pd.partner_name;
    
### to show district summary table
SELECT *
FROM District_summary;


### show partner summary table
SELECT *
